...
const webactionjs string = `function webactionRequestViaOptions(callbackurl,formid,actiontarget,command,enableProgressElem){
	postForm({
		url_ref:callbackurl,
		form_ref:formid,
		target:actiontarget,
...
func WebactionJS() io.Reader {
	return strings.NewReader(webactionjs)
}

func WebactionFindJS(webactionfindjs string) io.Reader {
	if strings.LastIndex(webactionfindjs, "/") >= 0 {
		webactionfindjs = webactionfindjs[strings.LastIndex(webactionfindjs, "/")+1:]
	}
	if webactionfindjs == "webaction.js" {
		return WebactionJS()
	}
	return nil
}
